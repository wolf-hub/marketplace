class UsersController < ApplicationController
	...
	def earnings
		@net_income = (Transaction.where("seller_id = ?", current_user.id).sum(:amount))
	...
	def withdraw
		...
		if amount <= 0
			...
			if transaction.save
				...
				flash[:alert] = "Cannot create request"
			end
		end
	...
	def current_user_params
		params.required(:user).permit(:from, :about, :status, :language, :avatar)
	end
end	