class GigsController < ApplicationController
  ...
  def create
    ...
    if @gig.save
      ...
      redirect_to request.referrer, flash: {error: @gig.errors.full_message}
    end
  end
  ...
  def update
    ...
    if @step==4 && @gig.photos.blank?
      return redirect_to request.referrer, flash: {error: "You don't have any photo"}
    end
    ...
    if @step==5
      ...
      if @gig.description.blank?
          ...
          return redirect_to edit_gig_path(@gig, step: 3), flash: {error: "You don't have any photo"}
      end  
    end
    ...
    if @gig.update(gig_params)
      ...
      return redirect_to request.referrer, flash: {error: @gig.errors.full_message}
    end
  ...
  def gig_params
    params.require(:gig).permit(:title, :description, :video, :active, :category_id, :has_single_pricing,
