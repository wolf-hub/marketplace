Trestle.resource(:users) do
  ...
  table do
     ...
     actions do |toolbar, instance, admin|
        toolbar.link 'Wallet', admin.path(:wallet, id: instance.id), class: "btn btn-danger"
  ...
  controller do
    ...
    def wallet
      @user = admin.find_instance(params)
      @net_income = (Transaction.where("seller_id = ?", @user.id).sum(:amount))
  ...
  search do |query|
    if query
      User.where("email LIKE ? OR full_name like ?", "%#{query}%", "%#{query}%")
    else
      User.all
    end
  end
